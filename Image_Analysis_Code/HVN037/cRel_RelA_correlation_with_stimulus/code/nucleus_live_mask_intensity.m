...
%     for t = 1:92
        ...
        path = '/Users/yijiachen/Documents/B_Cell_project/HVN037/cRel_RelA_correlation_sti/images/live_nucleus_mask/';
        addpath(path);

        filename_live_nucleus_mask = 'live_nucleus_mask.tiff';
        live_nucleus_mask=imread(filename_live_nucleus_mask);

        %% load CFP image
        path_2 = '/Users/yijiachen/Documents/B_Cell_project/HVN037/cRel_RelA_correlation_sti/images/raw/';
        addpath(path_2);
        ...
        for i = 1:length(info)
            CFP_img(:,:,i) = rgb2gray(imread(filename_CFP,i));
        end

        CFP_img_nlm = CFP_img;
        CFP_img_nlm(~live_nucleus_mask) = 0;

        savePath = '/Users/yijiachen/Documents/B_Cell_project/HVN037/cRel_RelA_correlation_sti/images/live_nucleus_mask/CFP_img_masked.tiff';
        imwrite(CFP_img_nlm, savePath);


        CFP_stats = regionprops("table",live_nucleus_mask,CFP_img_nlm,'Area',"Centroid",'MeanIntensity',"MajorAxisLength","MinorAxisLength",'PixelValues') ;
...
%         fn=['s', num2str(s)];
        ...
        for row_num = 1:size(CFP_stats,1)
            CFP_nucleus_live_mask_all_t.('nlm_medianint')(row_num,time_point)=median(cell2mat(CFP_stats.PixelValues(row_num,1)));
            CFP_nucleus_live_mask_all_t.('nlm_sumint')(row_num,time_point)=sum(cell2mat(CFP_stats.PixelValues(row_num,1)));
            CFP_nucleus_live_mask_all_t.('nlm_meanint')(row_num,time_point)=CFP_stats.MeanIntensity(row_num,1);
            CFP_nucleus_live_mask_all_t.('nlm_centroid')(row_num,((2*(time_point-1)+1):(2*(time_point-1)+1)+1))=CFP_stats.Centroid(row_num,1:2);
            CFP_nucleus_live_mask_all_t.('radius')(row_num,time_point)=radii_CFP(row_num,1);
            CFP_nucleus_live_mask_all_t.('area')(row_num,time_point)=CFP_stats.Area(row_num,1);
        end

        %% load YFP image
        %         filename_YFP = ['HVN037_Fluo_ALL_t', num2str(t,'%03.f'), 'c4_ORG', '.tif'];
        filename_YFP = 'YFP.tif';
        ...
        for i = 1:length(info)
            YFP_img(:,:,i) = rgb2gray(imread(filename_YFP,i));
        end

        YFP_img_nlm = YFP_img;
        YFP_img_nlm(~live_nucleus_mask) = 0;

        % Save the image as a TIFF file without reducing the resolution
       savePath = '/Users/yijiachen/Documents/B_Cell_project/HVN037/cRel_RelA_correlation_sti/images/live_nucleus_mask/YFP_img_masked.tiff';

        imwrite(YFP_img_nlm, savePath);


        YFP_stats = regionprops("table",live_nucleus_mask,YFP_img_nlm,'Area',"Centroid",'MeanIntensity',"MajorAxisLength","MinorAxisLength",'PixelValues') ;
        ...
        for row_num_2 = 1:size(YFP_stats,1)
            YFP_nucleus_live_mask_all_t.('nlm_medianint')(row_num_2,time_point)=median(cell2mat(YFP_stats.PixelValues(row_num_2,1)));
            YFP_nucleus_live_mask_all_t.('nlm_sumint')(row_num_2,time_point)=sum(cell2mat(YFP_stats.PixelValues(row_num_2,1)));
            YFP_nucleus_live_mask_all_t.('nlm_meanint')(row_num_2,time_point)=YFP_stats.MeanIntensity(row_num_2,1);
            YFP_nucleus_live_mask_all_t.('nlm_centroid')(row_num_2,((2*(time_point-1)+1):(2*(time_point-1)+1)+1))=YFP_stats.Centroid(row_num_2,1:2);
            YFP_nucleus_live_mask_all_t.('radius')(row_num_2,time_point)=radii_YFP(row_num_2,1);
            YFP_nucleus_live_mask_all_t.('area')(row_num,time_point)=YFP_stats.Area(row_num,1);
        end
...
% end
            ...
            save('/Users/yijiachen/Documents/B_Cell_project/HVN037/cRel_RelA_correlation_sti/data_saved/CFP_nucleus_live_mask_value.mat','CFP_nucleus_live_mask_all_t')
            save('/Users/yijiachen/Documents/B_Cell_project/HVN037/cRel_RelA_correlation_sti/data_saved/YFP_nucleus_live_mask_value.mat','YFP_nucleus_live_mask_all_t')
