...
%         imshow(YFP_img_bg_mask);
        ...
        savePath = sprintf('/Users/yijiachen/Documents/B_Cell_project/panel_cd/HVN032_cellpose_bg_mask/s%d/YFP_cellpose_bg_mask_%d.tiff', s, t);
        imwrite(YFP_img_bg_mask, savePath);

        %
        %         YFP_stats = regionprops("table",cellpose_mask_final,YFP_img_bg_mask,'Area','Centroid','PixelValues') ;
        %
        %
        ...
        %             YFP_wholecell_all_t.(fn).('wc_medianint')(row_num,time_point)=median(cell2mat(YFP_stats.PixelValues(row_num,1)));
        %             YFP_wholecell_all_t.(fn).('wc_sumint')(row_num,time_point)=sum(cell2mat(YFP_stats.PixelValues(row_num,1)));
        %             YFP_wholecell_all_t.(fn).('wc_centroid')(row_num,((2*(time_point-1)+1):(2*(time_point-1)+1)+1))=YFP_stats.Centroid(row_num,1:2);
        %         end
        ...
        % % %             %                     meanIntensity = floor(YFP_stats.MeanIntensity(i)); % Mean intensity value
        ...
        % % %             %                     areavalue = floor(YFP_stats.Area(i)); % Mean intensity value
        % % %             %                     % Add the mean intensity value as text at the centroid
        % % %             %                     text(x, y+15, num2str(areavalue), 'Color', 'yellow', 'FontSize', 10, 'FontWeight', 'bold');
        % % %             pixelvalue = sum(cell2mat(YFP_stats.PixelValues(i))); % Mean intensity value
    ...
    %     save('/Users/yijiachen/Documents/B_Cell_project/panel_cd/data_saved/HVN032_YFP_wholecell_value.mat','YFP_wholecell_all_t')
end

bg_mean_HVN032 = mean(YFP_img_bg_mask_nonzero_mean,'all');
bg_median_HVN032 = median(YFP_img_bg_mask_nonzero_median,'all');
save('/Users/yijiachen/Documents/B_Cell_project/panel_cd/data_saved/HVN032_YFP_bg_mean_median.mat','bg_mean_HVN032','bg_median_HVN032')
