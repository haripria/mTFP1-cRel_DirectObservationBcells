    ...
    @inbounds begin
        ...
        if phase == 1  # pre-simulation phase (no delay)
            ...
            if time > 1.5
                ...
                reactionFlux[TIKBD, SYNTHESIS] = Srates[TIKBD, BASALSYNTHESIS];
            end
        end
        # tIkB ----> 0 : Basal decay
        for i in TIKBA:TIKBD
        ...
        reactionFlux[TP50, BASALSYNTHESIS] = Srates[TP50, BASALSYNTHESIS] * hillInduction(cRelFeedback, cRelFeedback, concentration[NA50], concentration[NC50]; Kd = 150.0, hill = 1.0); # CHANGED from hillinduction of 10 to 5
        reactionFlux[TCREL, BASALSYNTHESIS] = Srates[TCREL, BASALSYNTHESIS] * hillInduction(cRelFeedback, cRelFeedback, concentration[NA50], concentration[NC50]; Kd = 150.0, hill = 1.0); # CHANGED from hillinduction of 10 to 5
        if phase == 1  # pre-simulation phase (no delay)
            for i in TRELA:TCREL
                reactionFlux[i, SYNTHESIS] = reactionFlux[i, BASALSYNTHESIS];
            end
        else
            reactionFlux[TRELA, SYNTHESIS] = reactionFlux[TRELA, BASALSYNTHESIS]; # no induced transcription for tRelA
            if time > 1
                reactionFlux[TP50, SYNTHESIS] = Srates[TP50, BASALSYNTHESIS] * hillInduction(cRelFeedback, cRelFeedback, hist_NA50_1, hist_NC50_1; Kd = 150.0, hill = 1.0); # CHANGED from hillinduction of 10 to 5
                reactionFlux[TCREL, SYNTHESIS] = Srates[TCREL, BASALSYNTHESIS] * hillInduction(cRelFeedback, cRelFeedback, hist_NA50_1, hist_NC50_1; Kd = 150.0, hill = 1.0); # CHANGED from hillinduction of 10 to 5
            else # transcription is at basal level
        ...
        reactionFlux[IKBDAA, DECAY] = reactionFlux[IKK, DEACTIVATION] * Srates[IKBD, DECAY] * concentration[IKBDAA] + Srates[IKBDAA, DECAY] * concentration[IKBDAA];
        ...
        reactionFlux[IKBDA50, DECAY] = reactionFlux[IKK, DEACTIVATION] * Srates[IKBD, DECAY] * concentration[IKBDA50] + Srates[IKBDA50, DECAY] * concentration[IKBDA50];
        ...
        reactionFlux[IKBD5050, DECAY] = reactionFlux[IKK, DEACTIVATION] * Srates[IKBD, DECAY] * concentration[IKBD5050] + Srates[IKBD5050, DECAY] * concentration[IKBD5050];
        ...
        reactionFlux[IKBDC50, DECAY] = reactionFlux[IKK, DEACTIVATION] * Srates[IKBD, DECAY] * concentration[IKBDC50] + Srates[IKBDC50, DECAY] * concentration[IKBDC50];
