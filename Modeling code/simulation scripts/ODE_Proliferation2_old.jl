...
function computeProlifFluxes!(concentration, reactionFlux, Srates, phase)
    @inbounds begin
        ...
        reactionFlux[TMYC, SYNTHESIS] = 5* Srates[TMYC, SYNTHESIS] * hillActivation(Ka = 1.0, na = 2.0, X = ((0.45*concentration[NA50] + 0.45*concentration[NC50] + 0.1*concentration[IKK])/MYCTHR));
        # NEW: fold-change detection
        reactionFlux[TMYC, REPRESSION] = hillRepression(Kr = 1.0, nr = 2.0, X = concentration[NP50P50]/P50THR);
        if MycRepression
            reactionFlux[TMYC, SYNTHESIS] = MYC_MOD * reactionFlux[TMYC, REPRESSION] * reactionFlux[TMYC, SYNTHESIS];
        end
        ...
        reactionFlux[TMYC, DECAY] = 5* Srates[TMYC, DECAY] * concentration[TMYC];
        # D03. tMyc ----> Myc : Synthesis by translation
        reactionFlux[MYC, TRANSLATION] = 4*EPS * Srates[MYC, TRANSLATION] * concentration[TMYC];
        ...
        reactionFlux[MYC, DECAY] = 3.5*Srates[MYC, DECAY] * concentration[MYC];
        ...
        if phase == 2 # to avoid cells dividing at the beginning of the stimulation phase (which they don't experimentally), 
                      # we don't compute the steady states for the downstream species to avoid progressing the cell cycle too much
