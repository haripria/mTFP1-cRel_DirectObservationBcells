...
# Define function to evaluate general Hill induction expression (2 float params)
#------------------------------------------------------------------
function hillInduction(w1::Float64, w2::Float64, X1::Float64, X2::Float64; Kd::Float64, hill::Float64)
    ans::Float64 = (1 + (w1 * abs(X1 / Kd) ^ hill) + (w2 * abs(X2 / Kd) ^ hill)) / (1 + (abs(X1 / Kd) ^ hill) + (abs(X2 / Kd) ^ hill));
    return ans;
end
...
function distribute_daughter_params(finalConcentration, rates, division_factor, which_daughter)
    ...
    @. New_rates[RELA:CREL, INITIALCONC] = ((@view New_rates[RELA:CREL, INITIALCONC]) + (@view New_rates[NRELA:NCREL, INITIALCONC])) / 2;
    @. New_rates[NRELA:NCREL, INITIALCONC] = (@view New_rates[RELA:CREL, INITIALCONC]);

    @. New_rates[AA:C50, INITIALCONC] = ((@view New_rates[AA:C50, INITIALCONC]) + (@view New_rates[NAA:NC50, INITIALCONC])) / 2;
    @. New_rates[NAA:NC50, INITIALCONC] = (@view New_rates[AA:C50, INITIALCONC]);
